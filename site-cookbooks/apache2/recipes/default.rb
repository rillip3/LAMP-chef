...
service "apache2" do
  ...
  when "centos","redhat","fedora","suse"
    ...
    restart_command "/sbin/service httpd stop; sleep 1; killall -9 httpd; /sbin/service httpd start; sleep 1"
    reload_command "/sbin/service httpd reload && sleep 2"
  when "debian","ubuntu"
    service_name "apache2"
    restart_command "/usr/sbin/invoke-rc.d apache2 stop; sleep 1; killall -9 apache2; /usr/sbin/invoke-rc.d apache2 start; sleep 1"
  ...
  if (node[:platform] == "ubuntu" && node[:platform_version].to_f > 13.04)
    ...
    file "#{node[:apache][:dir]}/status-htpasswd" do
      action :touch
    end
