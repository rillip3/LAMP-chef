...
class MySQL:
    def __init__(self):
        ...
        self.creds_file = get_conf_value(self.config_file, 'defaults-extra-file').split(',')
    ...
    def check_ping(self):
        try:
            ...
            if self.creds_file:
                for f in self.creds_file:
                    try:
                        status = subprocess.call(["/usr/bin/mysqladmin","--defaults-file="+f,"ping"], stdout=DEVNULL, stderr=DEVNULL)
                        if ping == 0:
                            break
                    except:
                        ping = 0
            elif self.user and self.password:
                ping = subprocess.call(["/usr/bin/mysqladmin","-u",self.user,"-p"+self.password,"ping"], stdout=DEVNULL, stderr=DEVNULL)
            else:
    ...
    def check_status(self):
        try:
            ...
            if self.creds_file:
                ...
                status = subprocess.call(["/usr/bin/mysqladmin","-u",self.user,"-p"+self.password,"status"])
            else:
